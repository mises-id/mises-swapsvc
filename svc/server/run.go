...
func NewEndpoints(service pb.SwapsvcServer) svc.Endpoints {
	// Business domain.
	...
	// Endpoint domain.
	var (
		syncswaporderEndpoint           = svc.MakeSyncSwapOrderEndpoint(service)
		swaporderpageEndpoint           = svc.MakeSwapOrderPageEndpoint(service)
		findswaporderEndpoint           = svc.MakeFindSwapOrderEndpoint(service)
		listswaptokenEndpoint           = svc.MakeListSwapTokenEndpoint(service)
		getswapapproveallowanceEndpoint = svc.MakeGetSwapApproveAllowanceEndpoint(service)
		approveswaptransactionEndpoint  = svc.MakeApproveSwapTransactionEndpoint(service)
		swaptradesEndpoint              = svc.MakeSwapTradesEndpoint(service)
...
func Run(cfg svc.Config) {
	...
	go func() {
		...
		s := grpc.NewServer()
		pb.RegisterSwapsvcServer(s, srv)
